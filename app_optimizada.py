...
if tabla.empty:
    ...
    with col_descarga_2:
        if st.button(
            "📦 Preparar apuestas Excel",
            key="preparar_apuestas_home",
            use_container_width=True,
            help="El Excel solo se genera cuando alguien lo solicita.",
        ):
            with st.spinner("Preparando el Excel de apuestas..."):
                st.session_state["excel_apuestas"] = generar_excel_apuestas_transparencia(
                    apuestas_df,
                    partidos,
                )

        if "excel_apuestas" in st.session_state:
            st.download_button(
                "📥 Descargar apuestas Excel",
                data=st.session_state["excel_apuestas"],
